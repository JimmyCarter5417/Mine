...
void CCfgMgr::GetMineInfo(uint& row, uint& col, uint& mine, def::ELevel& level)
{
	...
	level = (def::ELevel)GetPrivateProfileInt(g_strMine, g_strLevel, g_uDefLevel, m_strCfgPath);
}
void CCfgMgr::SetMineInfo(uint row, uint col, uint mine, def::ELevel level)
{
