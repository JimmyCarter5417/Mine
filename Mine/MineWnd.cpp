...
{   
    ...
    {       
        ...
        switch (m_uGameState)
        {
        case GAME_STATE_Wait: //等待或游戏中，正常修改m_posCurBlock方块状态
        case GAME_STATE_Run:
           {
...
void CMineWnd::OnLButtonUp(uint nFlags, CPoint pt)
{
    ...
    {       
        switch (m_uGameState)
        {
        case GAME_STATE_Wait: // 等待
        {
              m_uBtnState = BUTTON_STATE_Normal; // 按钮状态 
              m_uGameState = GAME_STATE_Run; // 开始游戏 
              m_uSpendTime = 0;   
              ...
              {
                  if (m_pBlockArea->GetOldState(m_posCurBlock) == BLOCK_STATE_Mine) // 点中雷
                  {
                  ...
                  {
                      if (m_pBlockArea->GetOldState(m_posCurBlock) == BLOCK_STATE_Normal) // 可打开
                      {
        ...
        case GAME_STATE_Run: // 游戏中
        {
            ...
            if (m_bLRBtnDown) // 左右鼠标同时按下
            {
                ...
                if ((m_pBlockArea->GetAroundMineNum(m_posCurBlock) == m_pBlockArea->GetAroundFlagNum(m_posCurBlock)) && // 周围的标记数等于雷数
                    (m_pBlockArea->GetCurState(m_posCurBlock) >= BLOCK_STATE_Num8)) // 方块必须已打开
                {
            ...
            {                               
                if (m_pBlockArea->GetOldState(m_posCurBlock) == BLOCK_STATE_Normal) // 点击左键之前该方块未作任何标记
                {
                ...
                else if (m_pBlockArea->GetOldState(m_posCurBlock) == BLOCK_STATE_Dicey) // 该方块标记为问号
                {
